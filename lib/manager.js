...
var _retryableError = require("./retryable-error");
/*
 *
...
const MAX_RETRIES_OFFLINE_MESSAGE = 'Max retries reached, transport in silent mode, OFFLINE';
const NON_RETRYABLE_OFFLINE_MESSAGE = 'Non-retryable connection error, transport in silent mode, OFFLINE';
class Manager extends _events.EventEmitter {
  ...
  isRetryableError(error) {
    ...
    return (0, _retryableError.isTransientConnectionError)(error);
  }
  ...
  onConnectionError(error) {
    if (this.shouldTryToReconnect(error)) {
      ...
      const offlineError = this.isRetryableError(error) ? new Error(MAX_RETRIES_OFFLINE_MESSAGE) : Object.assign(new Error(NON_RETRYABLE_OFFLINE_MESSAGE), {
        cause: error
      });
